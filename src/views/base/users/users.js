...
const columns = [
...

  {
    ...
    cell: row => {
      ...
      let status = row.additionalInfo?.verification_status;
      ...
      return (
        <div
          key={row.id + (row.additionalInfo?.verification_status ?? 'null')}
          className={className}
        >
          {statusText}
        </div>
...
const Users = () => {
  ...
  const handleOpenUpdateForm = () => {
    // open update modal, keep parent modal state as-is so we don't invoke onClose
    setUpdateModalVisible(true);
  };


// remove useCallback wrapper — a normal async function is fine
const handleUpdateUser = async (e) => {
  e.preventDefault();

  if (!userById || !userById.id) {
    console.error('No user selected to update (userById missing or id undefined)', userById);
  ...
  for (let key in updateUserData) {
    const originalValue = originalUserData ? originalUserData[key] : undefined;
  ...
  if (Object.keys(updatedFields).length > 0) {
    try {
      // extra log to ensure current userById at call time
      console.log("Updating user with fields:", userById, updatedFields);
  ...
  const handleUpdateAdditionalInfo = useCallback(async (e) => {
    ...
    // Defensive check: must have a selected user
    if (!userById || !userById.id) {
      console.error('No user selected for updating additionalInfo', userById);
      setError('No user selected to update additional info');
      return;
    }
  
    // Use a safe reference to existing additional info
    const existingAdditional = userById.additionalInfo ?? {};
    const updatedAdditionalFields = {};
  
    for (let key in updateAdditionalInfo) {
      const originalValue = existingAdditional[key];
      let updatedValue = updateAdditionalInfo[key];
  
      // preserve existing boolean handling you had
      if (typeof originalValue === 'boolean') {
        updatedValue = updatedValue === 'Yes';
      }
  
      // Only include changed and non-empty values
    ...
    if (Object.keys(updatedAdditionalFields).length > 0) {
      try {
        console.log('Updating additionalInfo for', userById.id, updatedAdditionalFields);
        await updateUser(userById.id, { additionalInfo: updatedAdditionalFields });
        setUpdateAdditionalInfoModalVisible(false);
        await fetchData();
        setError(null);
        // Optionally clear the selected user now that update is done:
        setUserById(null);
      } catch (err) {
        console.error('updateAdditionalInfo error:', err);
